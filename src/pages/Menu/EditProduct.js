  ...
  return (
    <div>
      <div className='flex__between'>
        <motion.h3 initial={{ x: 30 }} animate={{ x: 0 }} transition={{ duration: .5 }} className='heading'>تعديل المنتج {product.ar_name}</motion.h3>
      ...
      <motion.form initial={{ x: -30 }} animate={{ x: 0 }} transition={{ duration: .5 }} className="row" onSubmit={handleSubmit}>
        <div className="col-5 col-sm-12" style={{ display: 'flex', flexDirection: 'column' }}>
            ...
            <div className="form-group">
                <label for="number">الصنف</label> &nbsp;
                 <select required="required" onChange={handleChange} name="category">
                    ...
                    {
                        categories.map((c) => (
                            <option selected={+c.id === +product.category ? true : false} value={+c.id}>{c.ar_name}</option>
                        ))
                    }
                 </select>
            </div>
            ...
            <p>الحالي <a rel="noreferrer" target="_blank" href={currentImg}>{currentImg.split(`${process.env.REACT_APP_HOST_API}/uploads/`)}</a></p>
        </div>
